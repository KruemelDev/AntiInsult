            ...
            if not all_user == []:
                for i in range(0, len(all_user)):
                    if all_user[i] == (user_mention,):
                        embed = discord.Embed(title="Fehler", description="Der Benutzer ist bereits eingetragen.",
                                              color=discord.Color.red())
                        await message.channel.send(embed=embed)
                        return

                try:
                    mycursor.execute("INSERT INTO {} (allowed_user_name, allowed_user_id) VALUES (%s, %s)".format(
                        allowed_authors_table_name), (message.mentions[0].name, user_mention))
                    mydb.commit()
        ...
        if message.author.guild_permissions.administrator:
            ...
            for user in all_user:
                if user_mention in user:
                    mycursor.execute("DELETE FROM {} WHERE allowed_user_id=(%s)".format(allowed_authors_table_name),
                                     (user_mention,))
                    mydb.commit()
                        ...
                        if not list_of_insults == []:
                            for j in list_of_insults:
                                if addinsultlist[0] in j:
                                    ...
                                    mycursor.execute("INSERT INTO {} (insult) VALUES (%s)".format(insult_table_name),
                                                     (addinsultlist[0],))
                                    mydb.commit()
                                ...
                                if not list_of_insults == []:
                                    for j in list_of_insults:
                                        if addinsultlist[0] in j:
                                            ...
                                            mycursor.execute(
                                                "INSERT INTO {} (insult) VALUES (%s)".format(insult_table_name),
                                                (addinsultlist[0],))
                                            mydb.commit()
                                            embed = discord.Embed(title="Beleidigung hinzugefügt",
                                                                  description=f"Die Beleidigung '{addinsultword}' wurde hinzugefügt.",
                            ...
                            for j in list_of_insults:
                                if removeinsultlist[0] in j:
                                    embed = discord.Embed(title="Beleidigung gelöscht",
                                                          ...
                                                          color=discord.Color.red())
                                    await message.channel.send(embed=embed)
                                    mycursor.execute("DELETE FROM {} WHERE insult=(%s)".format(insult_table_name),
                                                     (removeinsultlist[0],))
                                    mydb.commit()
                                    return
                            embed = discord.Embed(title="Fehler",
                                ...
                                if list_of_insults == []:
                                    ...
                                    for j in list_of_insults:
                                        if removeinsultlist[0] in j:
                                            ...
                                            await message.channel.send(embed=embed)
                                            mycursor.execute(
                                                "DELETE FROM {} WHERE insult=(%s)".format(insult_table_name),
                                                (removeinsultlist[0],))
                                            mydb.commit()
                                            return
                                    embed = discord.Embed(title="Fehler",
